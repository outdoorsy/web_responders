...
type Response struct {
	...
	Fixer             Fixer
	Options           objx.Map
	ConditionalFields func() map[string]bool
...
func (response *Response) createStructResponse(value reflect.Value, depth int) interface{} {
	...
	if response.ConditionalFields != nil {
		condFields = response.ConditionalFields()
	}
	...
	for i := 0; i < value.NumField(); i++ {
		...
		default:
			...
			if condFields != nil && condParts != nil {
				for _, part := range condParts {
					si, ok := condFields[part]
					if ok {
						found = true
						if si {
							shouldInclude = true
